...
import { generateHandlerSource } from "./compile-intent.js";
...
export async function runInitCommand(options: InitOptions = {}): Promise<void> {
  ...
  // Write stub generated.ts and handler.ts so the server can start before first compile-intent run.
  const stubGeneratedPath = path.join(managedDirectory, "generated.ts");
  const stubHandlerPath = path.join(managedDirectory, "handler.ts");
  if (!fs.existsSync(stubGeneratedPath)) {
    const stubGenerated = [
      "/* eslint-disable */",
      "// Stub generated by `intent-compiler init`. Run `npm run compile-intent` to populate.",
      "",
    ...
    fs.writeFileSync(stubGeneratedPath, stubGenerated, "utf8");
  }
  if (!fs.existsSync(stubHandlerPath)) {
    fs.writeFileSync(stubHandlerPath, generateHandlerSource(config), "utf8");
  }
